...
class Character {
  constructor(name, src, xCanvas, yCanvas) {
    ...
    (this.direction = "Down"), (this.gameOver = false);
  }
  ...
  detectCollision = () => {
    game.jokers.forEach((joker, i) => {
      this.batrangs.forEach((batrang) => {
        if (
          joker.xCanvas < batrang.x + batrang.width &&
      ...
      if (
        joker.xCanvas < batman.xCanvas + batman.widthCanvas &&
...
function gameOver() {
  ...
  for (let i = 0; i < 5; i++) {
    setTimeout(function () {
      document.querySelector(".container").innerHTML = `<div>restart in ${
        5 - i
      } seconds</div>`;
      if (i === 4) {
...
class Batrang {
  constructor(x, y, width, height, color, direction, index) {
    ...
    this.height = height;
    this.color = color;
    (this.direction = direction), (this.index = index);
  }
  drawThisBatrang = () => {
    ...
    batman.batrangs.forEach((batrang, i) => {
      if (
        batrang.x < 0 ||
        batrang.x > canvas.width ||
        batrang.y < 0 ||
        batrang.y > canvas.height
      ) {
